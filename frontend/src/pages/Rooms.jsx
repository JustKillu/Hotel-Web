...
const HotelRooms = () => {
    ...
    const [roomSpaceFilter, setRoomSpaceFilter] = useState(''); 
    ...
    const handleReviewSubmit = (roomId) => {
        ...
        const date = new Date();
        const review = { text: reviews[roomId], user: user, date: date };
    
        axios.post(`http://localhost:3001/rooms/${roomId}/reviews`, review)
            .then(response => {
                console.log('Review enviada: ', response.data);
                setError(null);
                setRoomReviews(prevReviews => [...prevReviews, review]);
            })
            .catch(error => {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-10 overflow-auto">
                    <div className="bg-white p-6 rounded flex flex-col md:flex-row m-4 max-h-full overflow-auto">
                        ...
                        <div className="ml-0 md:ml-4">
                            ...
                            <p className="mb-2"><strong>Espacio:</strong>{selectedRoom.espacio}</p>
